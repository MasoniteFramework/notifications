...
class WelcomeNotification(Notifiable):
    ...
    def mail(self):
        return (
            ...
            .view(
                "/src/masonite/notifications/snippets/mail/heading",
    ...
    def slack(self):
        return (
            ...
            .dry()
            .text(
                "Masonite Notification: Read The Docs!, https://docs.masoniteproject.com/"
            )
            .channel("#bot")
...
class MockMailConfig:
    ...
    FROM = {"address": "hello@example.com", "name": "Masonite"}
...
class TestNotifiable(TestCase):
    ...
    def test_notification_sets_protected_members(self):
        assert self.notify.mail(WelcomeNotification, to="test@email.com") is None
        assert self.notify.called_notifications[0]._to == "test@email.com"

    def test_mail_notification_appends_template(self):
        assert self.notify.mail(WelcomeNotification, to="test@email.com") is None
        assert (
            "We greatly value your service!"
            in self.notify.called_notifications[0].template
        )

    def test_mail_notification_should_queue(self):
        assert (
            self.notify.mail(ShouldQueueWelcomeNotification, to="test@email.com")
            is None
        )

    def test_can_send_with_via_method(self):
        notifications = (
            self.notify.via("mail")
            .send(ShouldQueueWelcomeNotification, to="test@email.com")
