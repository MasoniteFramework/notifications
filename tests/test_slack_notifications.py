...
from src.masonite.notifications.exceptions import NotificationFormatError, SlackChannelArchived, SlackChannelNotFound, SlackInvalidMessage


webhook_url = "https://hooks.slack.com/services/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"
...
class TestSlackNotifications(TestCase):
    ...
    def test_notification_should_implements_to_slack(self):
        ...
        self.assertEqual("Notification model should implement to_slack() method.",
                         str(err.exception))
    ...
    def test_slack_notification_can_fail_silently(self):
        responses.add(responses.POST, webhook_url, body=b'channel_not_found', status=404)
        user = self.user()
    ...
    def test_sending_to_archived_channel(self):
        responses.add(responses.POST, webhook_url, body=b'channel_is_archived', status=403)
        user = self.user()
    ...
    def test_sending_too_many_attachments(self):
        responses.add(responses.POST, webhook_url, body=b'too_many_attachments', status=400)
        user = self.user()
        with self.assertRaises(SlackInvalidMessage):
            user.notify(WelcomeNotification())
